...
function CreateComboBox(parent, triggerWidth, triggerHeight, maxVisibleOptions, optionsData, optionHeight, triggerAnchor, triggerAnchorParent, triggerOffsetX, triggerOffsetY)
    ...
    -- Parent to UIParent for proper z-order (appears above other elements)
    dropdownContainer = CreateEmptyWindow("dropdownContainer" .. comboBoxId, "UIParent")
    ...
    local menuBackground = dropdownContainer:CreateColorDrawable(0, 0, 0, 0.8, "background")
    ...
    scrollButtonsContainer = CreateEmptyWindow("scrollButtonsContainer" .. comboBoxId, "UIParent")
    ...
    local scrollBackground = scrollButtonsContainer:CreateColorDrawable(0, 0, 0, 0.6, "background")
    ...
    for i = 1, totalOptions do
        ...
        option:SetHandler("OnEnter", function(self)
            ...
            local currentColor = UIParent:GetFontColor("btn_ov")
            self:SetTextColor(currentColor[1], currentColor[2], currentColor[3], currentColor[4])
        end)

        option:SetHandler("OnLeave", function(self)
            self.drawable:SetColor(0, 0, 0, 0.5)
            if selectedOptionText == self:GetText() then
    ...
    function dropdownTrigger:OnClick()
        ...
        if dropdownVisible then
            dropdownContainer:Raise()
        end
        scrollButtonsContainer:Show(dropdownVisible and totalOptions > maxVisibleOptions)
        if dropdownVisible and totalOptions > maxVisibleOptions then
            scrollButtonsContainer:Raise()
        end
